    ...
    return (
      <div className="side" id={this.props.location}>
        ...
        <div>
          ...
          <form onSubmit={this.handleChoose} onChange={this.clearError}>
            {this.props.items.map((item) => (
              <div key={item}>
                <input
                  ...
                  name={item}
                  value={item}
                  disabled={this.props.currentSide !== this.props.location}
                  defaultChecked={item === "traveler"}
                />
                <label>{emojis[item] + item}</label>
              </div>
            ))}
            <button disabled={this.props.currentSide !== this.props.location}>
              Move {this.props.location === "left" ? "right" : "left"}
            </button>
          </form>
        </div>
      </div>
